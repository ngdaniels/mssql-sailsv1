...
var sql = {
	...
	escape: function(val, stringifyObjects, timeZone, attrName) {
	...
	insertQuery: function (collectionName, data, primaryKey) {
		var tableName = collectionName;
    	return 'INSERT INTO ' + tableName  + ' (' + sql.attributes(collectionName, data) + ')' + ' VALUES (' + sql.values(collectionName, data) + '); SELECT SCOPE_IDENTITY() AS ['+primaryKey+']';
	},
    ...
    var type = sqlTypeCast(attribute.columnType);
		...
		// Process autoIncrement field
		if (attribute.autoIncrement) {
			return attrName + ' INT NOT NULL AUTO_INCREMENT';
		}
	...
	prepareCriterion: function(collectionName, value, key, parentKey) {
		...
		if (parentKey) {
			...
			else if (key === '!' || key === 'not' || key === '!=' || key === 'nin') {
	...
	prepareValue: function(collectionName, value, attrName) {
		...
		return sql.escape(value, undefined, undefined, attrName);
	},
	...
	serializeOptions: function(collectionName, options) {
		...
		if (options.sort && !options.skip && options.sort.length) {
			queryPart += 'ORDER BY ';

			// Sort through each sort attribute criteria
			_.each(options.sort, function(attr, direction) {
				_.each(attr, function(value, attrName) {
					queryPart += sql.prepareAttribute(collectionName, null, attrName) + ' ';
					
					queryPart += attr[attrName] + ', ';
				});
			});
...
function sqlTypeCast(type) {
	...
	switch (type) {
		...
		case '_numbertimestamp': return 'BIGINT';
...
function validSubAttrCriteria(c) {
	return _.isObject(c) && (
	!_.isUndefined(c.not) || !_.isUndefined(c.nin) || !_.isUndefined(c.greaterThan) || !_.isUndefined(c.lessThan) ||
	!_.isUndefined(c.greaterThanOrEqual) || !_.isUndefined(c.lessThanOrEqual) || !_.isUndefined(c['<']) ||
	!_.isUndefined(c['<=']) || !_.isUndefined(c['!']) || !_.isUndefined(c['!=']) || !_.isUndefined(c['>']) || !_.isUndefined(c['>=']) ||

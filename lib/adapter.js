	...
	*/
    ...
    // Check if this is an aggregate query and that there is something to return
      ...
      if (connections[connection].mssqlConnection) {
        if (connections[connection].mssqlConnection['1']) {
          // console.log(connections[connection].mssqlConnection[0]);

          var request = new mssql.Request(connections[connection].mssqlConnection['1']);
    ...
    join: function (connectionName, criteria, cb) { 
      ...
      }, function(error, parentRecords) {
      	for (var prop in criteria.instructions){
      		if (criteria.instructions[prop].strategy.strategy != 1)
      			continue;
      		for (var data of parentRecords){
      			if (data[prop] && data[prop].constructor == Array)
      				data[prop] = data[prop].length ? data[prop][0] : null;
      		}
      	}
      	return cb(error, parentRecords);
      });
    },
